...
import tensor.dtypes.{DType, NDSeq}
...
class TensorFloatTestModule extends Module {
    val tensor_temp = Tensor.empty(Seq(2, 2), () => new dtypes.Float(9, 7))
    val io = IO(new Bundle {
    ...
    val t1 = Tensor.Wire(Tensor.empty(Seq(2, 2), () => new dtypes.Float(9, 7)))
    val t2 = Tensor.Wire(Tensor.empty(Seq(2, 2), () => new dtypes.Float(9, 7)))
    t1 := io.in1
    t2 := io.in2
    val t3 = t1 + t2
